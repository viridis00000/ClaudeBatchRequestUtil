...
from src.python.anthropic_batch_request_util.batch_handler import AnthropicBatchHandler, BatchProcessingError, BatchStatus, AnthropicErrorType
from src.python.anthropic_batch_request_util.config.settings import settings
...
def _classify_task_error(error: Exception) -> TaskErrorType:
    ...
    if any(err_type in str(error.__class__) for err_type in [
...
def monitor_batch(
    ...
    try:
        ...
        return _handle_monitoring_error(e, job_id, result_path, start_time, current_time)
...
def _save_batch_results(
    ...
    try:
        ...
        logger.info(
            f"Successfully saved {len(results)} results to {result_path}"
        )
...
def _handle_monitoring_error(
    ...
    try:
        ...
        logger.error(
            f"Failed to handle monitoring error: {str(schedule_error)}", 
            exc_info=True
        )
        return {
            "status": "error",
...
class MonitoringState:
    ...
    def get_monitoring_metrics(
        ...
        avg_interval = (
            total_duration / self.monitoring_count 
            if self.monitoring_count > 0 else 0
        )
...
def _schedule_next_monitoring(
    ...
    try:
        ...
        task_metadata = _update_monitoring_state(
            task, job_id, current_status, error
        )
        ...
        if (
            hasattr(settings.worker.monitor, 'max_monitoring_time') and
            metrics.total_duration > settings.worker.monitor.max_monitoring_time
        ):
            raise TaskError(
                "Maximum monitoring time exceeded",
                error_type=TaskErrorType.PERMANENT
            )
